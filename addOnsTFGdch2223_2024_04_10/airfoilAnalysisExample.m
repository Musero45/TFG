...
%%% To extend the cl and cd curves to the whole [-pi,pi] we propose
%%% the Viterna-Janetzke model implemented in the functions cl2vjcl
%%% and cd2vjcd.

% It is required to obtain the lift coefficient between
% -pi and pi every degree.
% First obtain the cldata matrix using radians

d2r         = pi/180; 
clData(:,1) = d2r*clData(:,1);
cdData(:,1) = d2r*cdData(:,1);

% Transformation to 0-lift line as reference for angles of attack
alpha0      = 0;%-3.78*d2r;
clData(:,1) = clData(:,1)-alpha0;
cdData(:,1) = cdData(:,1)-alpha0;

% Second, define the angle of attack at which the lift coefficient should
% be extrapolated:
 aspan = linspace(-180,180,361)*d2r;

% Third, use cl2vjcl

AR = 3/20;
k = 0.6;
cl = cl2vjcl(clData,aspan,AR,k);
cd = cd2vjcd(cdData,aspan,AR);
...
hold on
grid on
legend('$\mathrm{Experimental}$','$\mathrm{V-J}$');
...
%%% Now we can construct a ARM airfoil for non-linear simulations

data =  struct('name','airfoiltestLSN',...
               'reference','[ref]',...
               'Mach',9999,...
               'Reynolds',9999,...
               'alphacl',aspan',...
               ...
               'cd',cd',...
               'alphacm',aspan',...
               'cm',9999*(ones(size(cl)))');
               

airfoiltest_ARM = singleAirfloil2ARMairfoilHe(data);

%save ('airfoiltestLSN_ARM','-struct','airfoiltest_ARM')

figure (1)

subplot(1,2,1)
plot(airfoiltest_ARM.ALPHA3D_CL(:,1,1)/d2r,airfoiltest_ARM.CL3D(:,1,1),'g.');
legend('$\mathrm{Experimental}$','$\mathrm{V-J}$','$\mathrm{3D}$');

subplot(1,2,2)
plot(airfoiltest_ARM.ALPHA3D_CD(:,1,1)/d2r,airfoiltest_ARM.CD3D(:,1,1),'g.');
legend('$\mathrm{Experimental}$','$\mathrm{V-J}$','$\mathrm{3D}$');