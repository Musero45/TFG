function missionOut = getSegmentGlideGammaT(he,atm,missIn,pos,varargin)
% getSegmentGlideGammaT solves the mission glide segment with given altiude 
% increment and descend angle.
%
% MissionOut is a structure with the following fields:
%
%   class: 'missionSegment'
%      id: 'Glide'
%  solver: @MYgetSegmentGlideGammaT
...
% The power state with gammaT2Vautorrotation

% Vglide = gammaT2Vautorrotation(he,fC,atm);

[Vglide,Pglide] = MYgammaT2Vautorrotation(he,fC,atm,...
                              'constrainedEnginePower','flightCondition');

fC.V  = Vglide;
fC.Vv = Vglide * sin(fC.gammaT);
fC.Vh = Vglide * cos(fC.gammaT);
P     = fC.P;
                        
% [VD,PD] = vGivenPower(he,fC,atm,'constrainedEnginePower',...
%                       'flightCondition');
% 
% fC.V  = VD;
% fC.Vv = VD * sin(fC.gammaT);
% fC.Vh = VD * cos(fC.gammaT);
% P     = fC.P;
