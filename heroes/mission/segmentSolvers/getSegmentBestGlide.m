function missionOut = getSegmentBestGlide(he,atm,missIn,pos,varargin)
% getSegmentBestGlide solves the mission glide segment with given 
% altiude increment and minimun fuel consumption.
% The glide is calculated for minimum descent angle corresponding to the GW
...
% Calculating the power state
                         ...
                         P = fC.P;
[gammaT_mgT VV_mgT VH_mgT] = gammaTminGammaT(he,fC,atm,options);


% Calculating the fuel mass consumption 

       missIn.mSeg{pos}.DT = missIn.mSeg{pos}.DH/VV_mgT;
                       DMf = he.engine.PSFC*P*missIn.mSeg{pos}.DT;
      missIn.mSeg{pos}.DMf = -DMf;
    missIn.mSeg{pos}.VV(1) = VV_mgT;
    missIn.mSeg{pos}.VV(2) = VV_mgT;
    missIn.mSeg{pos}.VH(1) = VH_mgT;
    missIn.mSeg{pos}.VH(2) = VH_mgT;
     missIn.mSeg{pos}.V(1) = sqrt( ( VV_mgT )^2 + ( VH_mgT )^2 );
     missIn.mSeg{pos}.V(2) = sqrt( ( VV_mgT )^2 + ( VH_mgT )^2 );
missIn.mSeg{pos}.gammaT(1) = gammaT_mgT;
missIn.mSeg{pos}.gammaT(2) = gammaT_mgT;
