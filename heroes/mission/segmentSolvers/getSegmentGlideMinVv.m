function missionOut = getSegmentGlideMinVv(he,atm,missIn,pos,varargin)
% getSegmentGlideMinVv solves the mission glide segment with given 
% altitude increment.
% The glide is calculated for minimum vertical velocity corresponding to the
% GW at the begining of the segment. 
%
% missionOut is a structure with the following fields:
%
%   class: 'missionSegment'
%      id: 'Glide'
%  solver: @MYgetSegmentGlideMinVv
...
%Calculating the power state
                         ...
                         P = fC.P;                         
[gammaT_mVV VV_mVV VH_mVV] = gammaTminVv(he,fC,atm,options);

%Calculating the fuel mass consumption 

    missIn.mSeg{pos}.DT = missIn.mSeg{pos}.DH/VV_mVV;
                    DMf = he.engine.PSFC*P*missIn.mSeg{pos}.DT;
   missIn.mSeg{pos}.DMf = -DMf;
 missIn.mSeg{pos}.VV(1) = VV_mVV;
 missIn.mSeg{pos}.VV(2) = VV_mVV;
 missIn.mSeg{pos}.VH(1) = VH_mVV;
 missIn.mSeg{pos}.VH(2) = VH_mVV;
  missIn.mSeg{pos}.V(1) = sqrt( ( VV_mVV )^2 + ( VH_mVV )^2 );
  missIn.mSeg{pos}.V(2) = sqrt( ( VV_mVV )^2 + ( VH_mVV )^2 );
missIn.mSeg{pos}.gammaT(1) = gammaT_mVV;
missIn.mSeg{pos}.gammaT(2) = gammaT_mVV;
