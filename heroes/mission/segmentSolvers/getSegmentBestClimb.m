function missionOut = getSegmentBestClimb(he,atm,missIn,pos,varargin)
% getSegmentBestClimb solves a mission climb segment with given 
% altitude increment, DH, the minimum fuel consumption and the maximum
% endurance velocity, VC.
% The climb is calculated for the vertical velocity that minimizes segment 
% energy, corresponding to the GW at the begining of the segment.
%
% missionOut is a structure with the following fields:
%
%   class: 'missionSegment'
%      id: 'Climb'
%  solver: @getSegmentBestClimb
...
[VC,PC] = vMaxEndurance(he,fc,atm,options);           
    
fc.V = VC;


%Calculating the power state
myfun  = @(Vv)ClimbEnergy(Vv,he,fc,atm,options);
mycon  = @(Vv)Pconstrain(Vv,he,fc,atm,options);


fc.Vh = fc.V;
[VHdummy,Vv0]    = vH4maxVv(he,fc,atm,options);
Vv               = fmincon(myfun,Vv0,[],[],[],[],[],[],mycon,options);
fc.Vv            = Vv;
fc.Vh            = sqrt(fc.V^2 - fc.Vv^2);
fc.gammaT        = atan(fc.Vv/fc.Vh);

EpS              = getEpowerState(he,fc,atm,options);

%Calculating the fuel mass consumption 

  missIn.mSeg{pos}.DT = missIn.mSeg{pos}.DH/EpS.Vv;
 DMf = he.engine.PSFC*EpS.P*missIn.mSeg{pos}.DT;
   missIn.mSeg{pos}.DMf = -DMf;
  missIn.mSeg{pos}.V(1) = EpS.V;
  missIn.mSeg{pos}.V(2) = EpS.V;
 missIn.mSeg{pos}.VV(1) = EpS.Vv;
 missIn.mSeg{pos}.VV(2) = EpS.Vv;
 missIn.mSeg{pos}.VH(1) = EpS.Vh;
 missIn.mSeg{pos}.VH(2) = EpS.Vh;
missIn.mSeg{pos}.gammaT(1) = EpS.gammaT;
missIn.mSeg{pos}.gammaT(2) = EpS.gammaT;
 ...
 %Additional Segment calculations
  ...
  missIn.mSeg{pos}.P(1) = EpS.P;
  missIn.mSeg{pos}.P(2) = EpS.P;
...
function E = ClimbEnergy(Vv,he,fc,atm,options)

fc.Vv     = Vv;
V         = fc.V;
Vh        = sqrt(V^2-Vv^2);
fc.Vh     = Vh;
fc.gammaT = atan(Vv/fc.Vh);
...
function [C,Ceq] = Pconstrain(Vv,he,fc,atm,options)
