function missionOut = getSegmentGlideMinV(he,atm,missIn,pos,varargin)
% getSegmentGlideMinV solves the mission glide segment with given
% altitude increment.
% The glide is calculated for minimum velocity corresponding to the GW at 
...
% Calculating the power state
                      ...
                      P = fC.P;                        
[gammaT_mV VV_mV VH_mV] = gammaTminV(he,fC,atm,options);


% Calculating the fuel mass consumption 

       missIn.mSeg{pos}.DT = missIn.mSeg{pos}.DH/VV_mV;
                       DMf = he.engine.PSFC*P*missIn.mSeg{pos}.DT;
    missIn.mSeg{pos}.DMf   = -DMf;
    missIn.mSeg{pos}.VV(1) = VV_mV;
    missIn.mSeg{pos}.VV(2) = VV_mV;
    missIn.mSeg{pos}.VH(1) = VH_mV;
    missIn.mSeg{pos}.VH(2) = VH_mV;
     missIn.mSeg{pos}.V(1) = sqrt( ( missIn.mSeg{pos}.VV(1) )^2 + ( missIn.mSeg{pos}.VH(1) )^2 );
     missIn.mSeg{pos}.V(2) = sqrt( ( missIn.mSeg{pos}.VV(2) )^2 + ( missIn.mSeg{pos}.VH(2) )^2 );
missIn.mSeg{pos}.gammaT(1) = gammaT_mV;
missIn.mSeg{pos}.gammaT(2) = gammaT_mV;
