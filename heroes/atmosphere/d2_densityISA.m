function d2_density = d2_densityISA(atmosphereData,h)
%d2_densityISA computes the 2nd derivative of the air density with 
%   respect to altitude as a function of altitude.
%
%   d2_density = d2_densityISA(atmosphereData,h) returns the 2nd derivative of 
...
d2_density = zeros(size(h));
n          = numel(h);
for j = 1:n
    if h(j) <= 11000
        %Derivative in the troposphere
        d2_density(j) = rho0*(-g/(R_g*alpha_T)-1)*(-g/(R_g*alpha_T)-2)...
                          *(alpha_T/T0)^2*(1+alpha_T*h(j)/T0)...
                          ^(-g/(R_g*alpha_T)-3); 
    else
        %Derivative in the stratosphere
        d2_density(j) = rho11*(-g/(R_g*T11))^2*...
