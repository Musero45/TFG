function delta = deltaISA(atmosphereData,h)
%deltaISA computes the value of the air pressure ratio to sea level 
%         pressure as a function of altitude.
%
%   delta = deltaISA(atmosphereData,h) returns the pressure ratio to sea
%   level pressure as a function of altitude according to the 
...
for j = 1:n
    if h(j) <= 11000
        ...
        delta(j) = (1+alpha_T*h(j)/T0) ^ (-g/(R_g*alpha_T)); 
    else
        %Pressure in the stratosphere
        delta(j) = p11 / p0 * exp(-g*(h(j)-11000)/(R_g*T11)); 
    end
end
