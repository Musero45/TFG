function sigma = sigmaISA(atmosphereData,h)
%sigmaISA computes the value of the air density ratio to sea level density
% as a function of altitude.
%
%   sigma = sigmaISA(atmosphereData,h) returns the density ratio 
%   to sea level density at an altitude according to the data specified 
...
for j = 1:n
    if h(j) <= 11000
        ...
        sigma(j) = (1+alpha_T*h(j)/T0) ^ (-g/(R_g*alpha_T)-1); 
    else
        %Density in the stratosphere
        sigma(j) = rho11 / rho0 * exp(-g*(h(j)-11000)/(R_g*T11)); 
    end
end