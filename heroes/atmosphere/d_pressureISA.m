function d_pressure = d_pressureISA(atmosphereData,h)
%d_pressureISA computes the derivative of the air pressure respect to 
%   altitude as a function of altitude.
%
%   d_pressure = d_pressureISA(atmosphereData,h) returns the derivative of 
...
d_pressure = zeros(size(h));
n          = numel(h);
for j = 1:n
    if h(j) <= 11000
        %Derivative in the troposphere
        d_pressure(j) = p0 * (-g/(R_g*alpha_T)) * (alpha_T/T0) *...
            (1+alpha_T*h(j)/T0) ^ (-g/(R_g*alpha_T)-1); 
    else
        %Derivative in the stratosphere
        d_pressure(j) = p11 * (-g/(R_g*T11)) *...
