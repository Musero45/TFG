...
%Creacion de los parametros adimensionales para las ecuaciones
    ...
    DmuYvect = linspace(-0.005,0.005,10);
    
       for DmuYi = 1:length(DmuYvect);
...
MHAdim.Analisis.ModeloAcciones = 'ElasticoNL';
    ...
    CFRPinA_ElasticoNLVec(DmuYi,:) = CFRPinA_ElasticoNL';
    CMRPinA_ElasticoNLVec(DmuYi,:) = CMRPinA_ElasticoNL';
    CMRalterElasticoNLVec(DmuYi,:) = CMRalterElasticoNL;
    beta_ElasticoNLVec(DmuYi,:) = beta_ElasticoNL*180/pi';
    lambda_iP_ElasticoNLVec(DmuYi) = lambda_iP_ElasticoNL';
    ...
    MindOinFVec(DmuYi,:) = MindOinF';
       
    
       end
    
     CFRx = @(x) interp1(DmuYvect,-CFRPinA_ElasticoNLVec(:,1),x);
     CFRy = @(x) interp1(DmuYvect,CFRPinA_ElasticoNLVec(:,2),x);
     CFRz = @(x) interp1(DmuYvect,-CFRPinA_ElasticoNLVec(:,3),x);
     
     CMRx = @(x) interp1(DmuYvect,-CMRPinA_ElasticoNLVec(:,1),x);
     CMRy = @(x) interp1(DmuYvect,CMRPinA_ElasticoNLVec(:,2),x);
     CMRz = @(x) interp1(DmuYvect,-CMRPinA_ElasticoNLVec(:,3),x);
     
     MindOinFVecx = @(x) interp1(DmuYvect,MindOinFVec(:,1),x);
     MindOinFVecy = @(x) interp1(DmuYvect,MindOinFVec(:,2),x);
     MindOinFVecz = @(x) interp1(DmuYvect,MindOinFVec(:,3),x);
     
     beta0 = @(x) interp1(DmuYvect,beta_ElasticoNLVec(:,1),x);
     beta1c = @(x) interp1(DmuYvect,beta_ElasticoNLVec(:,2),x);
     beta1s = @(x) interp1(DmuYvect,beta_ElasticoNLVec(:,3),x);
     
     lambdai = @(x) interp1(DmuYvect,lambda_iP_ElasticoNLVec,x);
