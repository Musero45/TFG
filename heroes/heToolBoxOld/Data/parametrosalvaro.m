...
%DATOS DEL ROTOR PRINCIPAL
    OmegaRotor = 260*2*pi/60;
    b_rp = 4;                     %nº de palas;
    R_rp = 17.13/2;               %Radio del principal
    c_rp = 0.54;%0.59;                  %cuerda

    Ip = 1280;          %Momento principal de la pala
    e_rp = 0*0.0017*R_rp;     %Excentricidad de la pala
    k_beta = 110000;
    Mp = 120;
    xGB = 3.2;
...
%DATOS DEL ROTOR ANTIPAR
    ...
    %posicionamiento respecto de los ejes fuselaje
        ...
        lRAV = 1.587;                 %distancia según eje Z
...
%DATOS DE LOS ESTABILIZADORES
    %Estabilizadores horizontales
        ...
        SIzquierdo = 0*Sestabhorizontal*(2/4); %(CAMBIADO ALVARO)
        c_eh=0.7;       %Cuerda de los estabilizadores
        
        theta_ehi = -5*pi/180;     %Ángulos de calado
        theta_ehd = -5*pi/180;
    ...
    %Estabilizador vertical
        SVertical = 4;%1.9;          %Superficie del estabilizador
        c_ev = 1.337;             %cuerda
        theta_ev = 0*pi/180;   %ángulo de calado

        %posicionamiento respecto de los ejes fuselaje
            lEVH = 3;%10;       %distancia según eje X
