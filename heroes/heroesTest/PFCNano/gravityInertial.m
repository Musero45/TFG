...
hold on
for j = 1:m
    plot(real(d(2:9,:,j)),imag(d(2:9,:,j)),mark{j},'Markersize',4,'MarkerFaceColor','none');
end
grid on
axis([-15 .5 0 5])
axis 'autox'
xlabel('Re (s) [1/s]')
ylabel('Im (s) [1/s]')
name = strcat('gMapCG',num2str(j+1));
savePlot(gcf,name,{'pdf'});


pairs = {};
%%% Uncomment following line in order to generate output files
pairs = {'format' {'pdf'} 'prefix' 'g' 'closePlot' 'no'};

legend = {'base' 'G_T' 'helisim'};
