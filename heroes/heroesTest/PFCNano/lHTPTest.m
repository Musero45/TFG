...
for j = 1:m
ndHe.geometry.ndllHTP = l(j)/R;
ndHe.geometry.ndlrHTP = l(j)/R;
...
xlabel('Re (s) [1/s]')
ylabel('Im (s) [1/s]')
name = strcat('eigMapHTP',num2str(j+1));
savePlot(gcf,name,{'pdf'});

axis([0 .4 0.2 .6])
xlabel('Re (s) [1/s]')
ylabel('Im (s) [1/s]')
text('Position',[0.03 .225],'string','l_{ht}= 6 m')
text('Position',[.13 .27],'string','l_{ht}= 4.5 m')
text('Position',[.27 .325],'string','l_{ht}= 3 m')
name = strcat('eigMapHTP',num2str(j+2));
savePlot(gcf,name,{'pdf'});
