...
ThetaA = -.01/.3*(k-1);
    ...
    legend = {'S_{\beta}=0.4','S_{\beta}=0.25','S_{\beta}=0.1'};
    plotSimpAMPract(aeromechanicSprefixtate,legend,{'format' {'pdf'} 'prefix' prefix{k} 'closePlot' 'yes'});
    
    legend = {'S_{\beta}=0.4','S_{\beta}=0.25','S_{\beta}=0.1','S_{\beta}=0.4 Simp','S_{\beta}=0.25 Simp','S_{\beta}=0.1 Simp'};
    r2d    = 180/pi;
    ...
    plotOpt.mode = 'thick';
    plotOpt.prefix = prefix{k};
    plotOpt.format = {'pdf'};
%    plotOpt.mark = {'k-', 'r-', 'b-','k-.', 'r-.', 'b-.','k--', 'r--', 'b--'};
    plotCellOfStructures(simpAMState,axesData,legend,plotOpt);
end
t(1) = toc;

flightConditionT = [0; 0; 0.01; 0; 0; 0];
flightConditionA = [0; 0; -0.01; 0; 0; 0];
muxA = 0:.01:.3;
...
axesData.ylabs = {'\Omega?d\beta_{1C}/d\omega_x [-]' '\Omega?d\beta_{1C}/d\omega_y [-]'...
    ...
    '\Omega?d\beta_{1S}/d\omega_z [-]' 'd\beta_{1C}/d\theta_{1C} [-]' '\partial \beta_{1C}/ \partial \theta_{1S} [-]'...
...
% pairs = {'format' {'pdf'} 'prefix' 'deriv' 'closePlot' 'yes'};

plotOpt = parseOptions(pairs,@setHeroesPlotOptions);
plotOpt.mode = 'thick';
%plotOpt.mark = {'k-', 'r-', 'b-','k-.', 'r-.', 'b-.',};
