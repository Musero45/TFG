function io = inflowFrameTest(mode)
...
for k =1:length(sideslip)
    ...
    figure(k)
    surfc(xA,yA,indV,'EdgeColor','none');
    xlabel('x_A [-]'); ylabel('y_A [-]'); zlabel('\lambda_i [-]');
    name = strcat(int2str(k),'_surf');
    savePlot(gcf,name,{'pdf'});

    figure(k+length(sideslip))
    [C,h]=contour(xA,yA,indV,'k','LineWidth',2);
    clabel(C,h);
