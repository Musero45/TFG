function [ax,lx] = plotStabilityEigencharacteristics(coswmf,AXDS,AYDS,varargin)
...
modeparams   = options.eigenCharNames;
nm           = length(modeparams);
...
figure(hgcf + 1)
hold on;
set(gcf,'Name',modeparams{j});
for i = 1:8
    plot(xdata,w(:,i),options.mode8liner{i}); hold on;
end
xlabel(AXDS.lab);ylabel(options.eigenCharLabs{j});
legend(options.eigenText,'Location','Best');
