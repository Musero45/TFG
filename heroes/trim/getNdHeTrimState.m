function ndTrimState = getNdHeTrimState(ndHe,muWT,FC,varargin)
% getNdHeTrimState provides the trim state of a non dimensional
% helicopter.
%
% ndTs = getNdHeTrimState(ndHe,muWT,FC,varargin) provides the 
% trim state structure:
%
%        class: 'ndTrimState'
%     solution: [1x1 struct]
%    residuals: [1x1 struct]
%        flags: [n1xn2xn3xn4xn5 double]
%      actions: [1x1 struct]
%          vel: [1x1 struct]
%        ndPow: [1x1 struct]
%
% for a given non dimensional helicopter, ndHe, a nondimensional wind
% vector expressed in the ground reference system, muWT, a flight codition
% FC and set of given options in varargin.
%
% getNdHeTrimState solves the non linear system of equations of the 
% general trim problem. 
%
...
% The inputs are:
%
% ndHe:      is a non dimensional helicopter
%
% See also rigidCanonical, rigidHe2ndHe.
%
...
%        ndPow: [1x1 struct]
%
% is formed by the substructres
%
% solution:         is a structuture with 43 fields 
%
...
%
% residuals: [1x1 struct]. Is a strucuture with 42 fields:
%
...
% 
%           with the residuals of the 42 trim equations corresponding
%           to each flight conidtion in FC
%
% flags: [n1xn2xn3xn4xn5 double]. Is a matrix with the exitflags
%           corresponding to the solution of the system of equations
%           of the trim problem for each flight condition in FC
%
%
% actions: [1x1 struct]. Is a structure with the force and moments
%           coefficients corresponding to each flight condition in FC.
%
%          actions is configured by the substructures 
%
%                   weight: [1x1 struct]
%                mainRotor: [1x1 struct]
%                tailRotor: [1x1 struct]
%                 fuselage: [1x1 struct]
%              verticalFin: [1x1 struct]
%                  leftHTP: [1x1 struct]
%                 rightHTP: [1x1 struct]
% 
%          and each of then has the substructures
%
%                 fuselage: [1x1 struct]
%                   ground: [1x1 struct]
%
%          containing the non dimensional coefficients expressed in
%          fuselage and ground reference systems. In each case the
%          corresponding structure is formed by the fields:
%
%                      CFx: [n1xn2xn3xn4xn5 double]
%                      CMx: [n1xn2xn3xn4xn5 double]
%                     CMFx: [n1xn2xn3xn4xn5 double]
%                     CMtx: [n1xn2xn3xn4xn5 double]
%                      CFy: [n1xn2xn3xn4xn5 double]
%                      CMy: [n1xn2xn3xn4xn5 double]
%                     CMFy: [n1xn2xn3xn4xn5 double]
%                     CMty: [n1xn2xn3xn4xn5 double]
%                      CFz: [n1xn2xn3xn4xn5 double]
%                      CMz: [n1xn2xn3xn4xn5 double]
%                     CMFz: [n1xn2xn3xn4xn5 double]
%                     CMtz: [n1xn2xn3xn4xn5 double]
%
%          where:
%
%         CFi: is the i-force coefficient
%         CMi: is the i-moment coefficient in the reference point of the
%              active element (Direct moment).
%        CMFi: is the i-moment coefficient in the center of mass of the
%              helicopter due to the i-force applied in the reference point
%              of the element
%        CMti: is the total i-moment in the center of mmas of the
%              helicopter due to the element.
%
%                           CMti = CMi+CMFi 
%
%    vel: [1x1 struct]. Is the structure witht the relevant
%         velocities in the reference points of each element. 
%         See also velocities.m
%
%        ndPow: [1x1 struct]. Is a structure containing the fields
%
%                   CQmr: [n1xn2xn3xn4xn5 double]
%                   CQtr: [n1xn2xn3xn4xn5 double]
%                   CPmr: [n1xn2xn3xn4xn5 double]
%                   CPtr: [n1xn2xn3xn4xn5 double]
%              CPtLossmr: [n1xn2xn3xn4xn5 double]
%              CPtLosstr: [n1xn2xn3xn4xn5 double]
%                CPtrans: [n1xn2xn3xn4xn5 double]
%                 etaTmr: [n1xn2xn3xn4xn5 double]
%                 etaTtr: [n1xn2xn3xn4xn5 double]
%                  etaTr: [n1xn2xn3xn4xn5 double]
%                   etaM: [n1xn2xn3xn4xn5 double]
%                     CP: [n1xn2xn3xn4xn5 double]
%                      P: [n1xn2xn3xn4xn5 double]
%
%        where
%
%       CQmr: is main rotor torque coefficient.
%       CQtr: is tail rotor torque coefficient.
%       CPmr: is the main rotor power coefficient.
%       CPtr: is tail rotor power coefficient.
%  CPtLossmr: is the main rotor trasmission losses power coefficient.
%  CPtLosstr: is the tail rotor trasmission losses power coefficient.
%    CPtrans: is the trasmission losses power coefficient.
%     etaTmr: is the ratio CPtLossmr/abs(CPmr)
%     etaTtr: is the ratio CPtLosstr/abs(CPtr)
%      etaTr: is the ratio CPtr/CPmr
%       etaM: is the ratio CP/CPmr;
%         CP: is the power coefficient
%          P: is the power in watts.
%
% All force, moment/torque and power coefficients are refered to actual
% main rotor variables R and OmegaRated*omega.
%
%==========================================================================
%
%==========================================================================
...
%--------------------------------------------------------------------------
%
%    ndTs = getNdHeTrimState(ndHe,muWT,FC) provides the general
%    trim state for nondimensional helicopter ndHe, non dimensional
...
%    provides the general trim state for nondimensional
...
%      muWT       = [0; 0; 0];
%
%     ndTs = getNdHeTrimState(ndHe,muWT,...
%                       {'VOR',0.1,...
%                       'gammaT',0,...
%                       'betaf0',0,...
%                       'vTOR',0,...
...
%     
%     and as an example the descend angle gammaT expressed in degrees
%     is presented
%
%      ndTs.solution.gammaT*180/pi
%
%       ans(:,:,1) =
%
...
%==========================================================================
%=====================================================================
%     EXAMPLE OF USAGE 6: Calculate the trim solution and trim
%     residual for a straight autorrotation descend flight
%     in ISA at sea level with nondimensional velocity component
%     uT/OR = [0.1 0.2],ratio of omegas: omega=OmegaAu/OmegaRated =
%     [0.95 1 1.05], slip angles betaf0 = [0 5*180/pi] and no wind.
%
%     With prescription of trim initial conditions: the function
%     ndTS       = getNdHeTrimState(ndHe,muWT,FC) calculates the
%     solution for the first flight conditions in FC using initial
%     conditions in for the helicoterTrim system of equations prescribed
%     inside the funtion getInitialTrimConditions. There is a possibility
%     to include usser defined initial conditions for the mentioned system
%     of equations through 
%
%                       options.IniTrimCon = S0
%
%     being S0 a GeneralTrimSolution structure with the expected 42 fields
%     corresponding to a solution of a general trim problem.
%
%     For instance as initial condition for this example, one can assume
%     that a solution of an engine-on level flight at uT/OR = 0.1 can be 
%     suitable, then the problem should be  
%
%     atm        = getISA;
%     he         = PadfieldBo105(atm);
%     ndHe       = rigidHe2ndHe(he,atm,0);
%     muWT       = [0; 0; 0];
%
%     ndTS0       = getNdHeTrimState(ndHe,muWT,...
%                       {'uTOR',0.1,...
%                       'gammaT',0,...
%                       'betaf0',0,...
%                       'vTOR',0,...
...
%--------------------------------------------------------------------
%=====================================================================
%     EXAMPLE OF USAGE 7: Calculate the trim state transitioning
%     from ascend vertical flight to descent vertical flight through
%     hover.
%
%      FC = {'VOR',linspace(0.0001,0.05,10),...
%      'gammaT',[-pi/2,pi/2],...
%      'cs',[0],...
%      'Psi',0,...
%      'vTOR',0 ...
%      };
%  
%     ndTrimState = getNdHeTrimState(ndHe,muWT,FC);
%
%    The alternative flight condition
%  
%      FC = {'wTOR',linspace(-0.05,0.05,20),...
%      'uTOR',0.00001,...
%      'cs',[0],...
%      'Psi',0,...
%      'vTOR',0 ...
...
if iscell(ndHe)
    ...
    for i = 1:n
        ndTrimState{i}    = myGetNdHeTrimState_i(ndHe{i},muWT,FC,options);
    end

else
    ndTrimState    = myGetNdHeTrimState_i(ndHe,muWT,FC,options);
end

function ndTrimState = myGetNdHeTrimState_i(ndHe,muWT,FC,varargin)

options = parseOptions(varargin,@setHeroesRigidOptions);
S0 = options.IniTrimCon;  
...
[iniTrimCon,~] = getInitialTrimConditions(ndHe,S0);                                                   
...
Solution.('class')  = 'GeneralNdHeTrimSolution';
Residuals.('class') = 'GeneralNdHeTrimResiduals';
...
%================================================================
 ...
 for i = 1:n
        ...
        trimSetUp = struct(...
                    'FC1',FC1,'FC2',FC2,'FC3',FC3,'FC4',FC4,'FC5',FC5,...
                    'valFC1',valFC1,'valFC2',valFC2,'valFC3',valFC3,...
                    'valFC4',valFC4,'valFC5',valFC5);

        system2solve = @(x)helicopterTrim(x,muWT,ndHe,trimSetUp,options);

        [x,FVAL,FLAG] = nlSolver(system2solve,initialCondition,options);
...
ndTrimState = getNdHePostProSolution(Solution,Residuals,Flags,muWT,ndHe,options);
